#!/usr/bin/env python3
"""Test script to verify LLM-based agent selection is working correctly."""
...
async def test_llm_agent_selection():
    """Test that the LLM-based agent selection works correctly."""
    
    print("🧪 Testing LLM-Based Agent Selection...")
    print("=" * 60)
    ...
    test_cases = [
        ...
        {
            "query": "Debug this JavaScript code: function test() { return x + y; }",
            "expected_agent": "code",
            "description": "Debugging question"
        },
        {
            "query": "Find information about the latest AI research papers",
            "expected_agent": "research",
            "description": "Information gathering"
        },
        {
            "query": "Create marketing copy for a new product launch",
            "expected_agent": "creative",
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"   Agents used: {agents_taken}")
                    ...
                    # Show the reasoning to see what the LLM decided
                    reasoning = result.get("reasoning", [])
                    if reasoning:
                        print(f"   Reasoning: {reasoning[0][:100]}...")
                    
                    # Show the first action to see what agent was called
                    if actions_taken:
                        first_action = actions_taken[0]
                        observation = first_action.get("observation", "")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("📊 LLM-BASED AGENT SELECTION TEST SUMMARY")
    print("=" * 60)
    ...
    if successful_tests == total_tests:
        print("   ✅ All LLM-based agent selections are working perfectly!")
    elif successful_tests > total_tests * 0.8:
        print("   ✅ LLM-based agent selection is working very well!")
    elif successful_tests > total_tests // 2:
        print("   ⚠️  LLM-based agent selection is working, but could be improved")
    else:
        print("   ❌ LLM-based agent selection needs significant improvement")
        print("   🔧 Consider refining the agent selection prompt or agent metadata")
